...
const Layout = ({ children }) => {
  ...
  return (
    <div className="min-h-screen bg-background text-foreground">
      <header className="sticky top-0 z-30 bg-[#faf7ee]/90 backdrop-blur border-b border-border">
        <div className="max-w-5xl mx-auto px-4 py-3 flex items-center justify-between">
          ...
          <div className="flex items-center gap-2">
            ...
            {!isPremium && (
              <Link to="/tutorial">
                <Button data-testid="tour-link" variant="outline" size="sm" className="gap-1">
                  <Sparkles className="w-4 h-4" /> Tour
                </Button>
              </Link>
            )}
            {user?.is_admin && (
      ...
      <main className={`max-w-5xl mx-auto px-4 py-5 md:pb-10 ${isPremium ? "has-bottom-nav" : ""}`}>
        {children}
      </main>

      {/* Mobile bottom nav — only premium */}
      {isPremium && (
